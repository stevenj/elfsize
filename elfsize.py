...
# This file USED TO BE a library called elffile.  I have repurposed it
# into a program to display the size of elf sections for embedded
# object file analysis.  The reason is obj-size sucks and can't show
# the used/available space of a device it doesn't know about.  This
# program doesn't care.  Tell it the maximum size of a section and 
# it will show percentage used.
...
class Coding(object):
    ...
    Closures by code::
        ...
        Peoplebycode[3].closure

    Or by name::

        Peoplebyname['Bob'].closure
    ...
    You can list all codes in this class::
        ...
        [code for code in Peoplebycode]

    Or all names::

        [name for name in Peoplebyname]

    You can test for inclusion by name::

        'Herman' in Peoplebyname
    ...
    Note that each subclass of Coding represents a unique type::
        ...
        assert isinstance(Peoplebycode[3], PeopleCoding)
...
def elfopen(name=None, fileobj=None, map=None, block=None):
    """
...
class ElfFile(StructBase):
    ...
    def _regen_section_name_table(self):
        ...
        p = 0
        section.contents[p] = b'\0'
        p += 1

        for sh in self.sectionHeaders:
            l = len(sh.name)
            section.contents[p:l] = sh.name
            section.nameoffset = p
            p += l
            section.contents[l] = b'\0'
            p += 1
...
def GetConfigBool(value, default):
    # If value is neither True or False, ignore and return default.
    if (value.lower() == 'true'):
        return True
...
def GetConfigNumber(value, default)    :
    # If its not a hex or decimal number, ignore and return default.
    try:
        return int(value,0)
    except:
        pass
...
def MemoryStartAddress(memlocation):
    return MEMORY_LOCATIONS[memlocation]['start']

def MemoryDataStartAddress(memlocation):
    return MEMORY_LOCATIONS[memlocation]['datastart']

def MemoryVStartAddress(memlocation):
    return MEMORY_LOCATIONS[memlocation]['vaddr']

def SectionAddress(section):
    return section['addr'] # Sorts on Virtual Address, but Virtual Address is in the same order as physical address anyway.

# Print iterations progress
def PercentageBar (used, total, prefix = '', suffix = '', decimals = 1, length = 32, fill = '█'):
    """
    Creates a Percentage Bar String
    @params:
        used        - Required  : Total currently used (Int)
        total       - Required  : Maximum value (Int)
        decimals    - Optional  : positive number of decimals in percent complete (Int)
        length      - Optional  : character length of bar (Int)
        fill        - Optional  : bar fill character (Str)
    """
    percent = ("{:{size}.{decs}f}").format(100 * (used / float(total)), size=decimals+4, decs=decimals)
    if (length == 0):
        return '{}%'.format(percent)

    filledLength = int(length * used // total)
    bar = fill * filledLength + '-' * (length - filledLength)
    return '[{}] {}%'.format(bar, percent)

def printRow(region, region_width, section, section_width, addr, size, used, pctwidth):
    print("{:<{region_width}} | {:<{section_width}} | {:>7} | {:>5} | {:>5} | {}".format(
                region,section, addr, size, used, 
                PercentageBar(used, size, length=pctwidth),
                region_width = region_width, section_width=section_width))

def main():

    parser = argparse.ArgumentParser(description='Dump Section Sizes from an Elf, and report percentage used (if known)')
    parser.add_argument('config', help = "A Configuration File defining the interesting sections.")
    parser.add_argument('elf', help = "The ELF file to dump.")

    args = parser.parse_args()

    if (not os.path.isfile(args.config)):
        print("ERROR: config file must point to an INI formatted file.  Can not find config file: {}".format(args.config))
        exit(1)

    if not os.path.isfile(args.elf):
        print("ERROR: elf file must point to an ELF format file.  Can not find elf file: {}".format(args.elf))
        exit(1)
    ...
    # Display Defaults
    DEVICE="REGION" # Default to a value that makes sense in the table
    BARLENGTH=32    # Default Percentage Bar Length
    ADDRESS_SORTER=MemoryStartAddress

    for section in config.sections():
        if (section != 'DISPLAY'):
            MEMORY_LOCATIONS[section]={
                "start" : None,
                "size"  : None,
                "vaddr" : None,
                "mapunit" : 1,
        ...
        for key in config[section]:
            if (section == 'DISPLAY'):
                if (key == 'device'):
                    DEVICE=config[section][key]      
                if (key == 'barlength'):
                    BARLENGTH=GetConfigNumber(config[section][key], BARLENGTH)
                if (key == 'sortby'):
                    if config[section][key].lower() == 'address':
                        ADDRESS_SORTER=MemoryStartAddress
                    if config[section][key].lower() == 'virtual':
                        ADDRESS_SORTER=MemoryVStartAddress
                    if config[section][key].lower() == 'data':
                        ADDRESS_SORTER=MemoryDataStartAddress
            else:
                MEMORY_LOCATIONS[section][key] = GetConfigNumber(config[section][key], None)
                if (key == 'start'):
                    if (MEMORY_LOCATIONS[section]['vaddr'] is None):
                        MEMORY_LOCATIONS[section]['vaddr'] = MEMORY_LOCATIONS[section][key]

                    if (MEMORY_LOCATIONS[section]['datastart'] is None):
                        MEMORY_LOCATIONS[section]['datastart'] = MEMORY_LOCATIONS[section][key]

    elf = elfopen(name = args.elf)._list_encode()

    for section in elf[2]['sectionHeaders']:
        if (section[2]['type'] != 'SHT_NULL'):
            for location in MEMORY_LOCATIONS:
                if ((section[2]['addr'] >= MEMORY_LOCATIONS[location]['vaddr']) and 
                    (section[2]['addr'] < MEMORY_LOCATIONS[location]['vaddr']+MEMORY_LOCATIONS[location]['size'])):
                    if ((section[2]['flags'] & 0x2) != 0): # Ignore any sections which don't end up in memory.
                        MEMORY_LOCATIONS[location]['elfdata'].append(section[2])
                    break

    # Size fields.
    region_field_size = len(DEVICE)
    section_field_size = 0
    for location in MEMORY_LOCATIONS:
        if len(location) > region_field_size:
            region_field_size = len(location)
        for section in MEMORY_LOCATIONS[location]['elfdata']:
            if len(section['name'].decode("utf-8")) > section_field_size:
                section_field_size = len(section['name'].decode("utf-8"))

    # print Title Bar of report
    if (BARLENGTH == 0):
        pct_title="Percent"
    else:
        pct_title="Percentage Used"

    print("{:<{region_width}} | {:<{section_width}} | Address |  Size |  Used | {}".format(
        DEVICE,"SECTION",pct_title,region_width=region_field_size, section_width=section_field_size))

    print("{:-<{region_width}}-+-{:-<{section_width}}-+---------+-------+-------+{:-<{pct_width}}".format(
        "","","", region_width=region_field_size, section_width=section_field_size, pct_width=BARLENGTH+10))

    for location in sorted(MEMORY_LOCATIONS, key=ADDRESS_SORTER):
        if len(MEMORY_LOCATIONS[location]['elfdata']) == 0:
            # Show Empty Memory Section
            printRow(location, region_field_size, 
                     "", section_field_size,
                     "{0:#0{1}x}".format(MEMORY_LOCATIONS[location]['start'],6), 
                     MEMORY_LOCATIONS[location]['size'], 
                     used=0, 
                     pctwidth=BARLENGTH)

        elif len(MEMORY_LOCATIONS[location]['elfdata']) == 1:
            # Show Memory Section with 1 elf section
            printRow(location, region_field_size, 
                     MEMORY_LOCATIONS[location]['elfdata'][0]['name'].decode("utf-8"), section_field_size,
                     "{0:#0{1}x}".format(MEMORY_LOCATIONS[location]['start'],6), 
                     MEMORY_LOCATIONS[location]['size'], 
                     used=MEMORY_LOCATIONS[location]['elfdata'][0]['section_size'], 
                     pctwidth=BARLENGTH)
            # Check if memory is blocked, if so show blocks USED and FREE also.  Such as Flash.
            if (MEMORY_LOCATIONS[location]['page'] > 1):
                printRow("", region_field_size, 
                        "PAGES", section_field_size,
                        "{0:#0{1}x}".format(MEMORY_LOCATIONS[location]['start'],6), 
                        MEMORY_LOCATIONS[location]['size']//MEMORY_LOCATIONS[location]['page'], 
                        used=math.ceil(MEMORY_LOCATIONS[location]['elfdata'][0]['section_size']/MEMORY_LOCATIONS[location]['page']), 
                        pctwidth=BARLENGTH)                                 
        else:
            # Show Memory Section with MULTIPLE elf sections.
            region = location
            totalused = 0
            for section in sorted(MEMORY_LOCATIONS[location]['elfdata'], key=SectionAddress):
                totalused += section['section_size']
                paddr = section['addr'] - MEMORY_LOCATIONS[location]['vaddr'] + MEMORY_LOCATIONS[location]['start']
                printRow(region, region_field_size, 
                         section['name'].decode("utf-8"), section_field_size,
                         "{0:#0{1}x}".format(paddr,6),
                         MEMORY_LOCATIONS[location]['size'], 
                         used=section['section_size'], 
                         pctwidth=BARLENGTH)
                region=""  # Only show region on first line.
            printRow("", region_field_size, 
                     "TOTAL", section_field_size,
                     "{0:#0{1}x}".format(MEMORY_LOCATIONS[location]['start'],6),
                     MEMORY_LOCATIONS[location]['size'], 
                     used=totalused, 
                     pctwidth=BARLENGTH)
